...
Optimize for branch lengths, nucelotide substitution rates and tree topology
 simultaneously.
...
Bootstrap on optimised model. The program resampled the nucleotides in the
 alignment (columns in the input matrix).
...
`Mme_S` is more likely to affect patient `Mme_L`. The bootstrap support is 100%
 (in my run), suggesting `Mme_S` and `Mme_L` share a more recent ancestor
  (closer relationship) than `Mr D`
