...
```{r}
compute_resposibilities <- function(obs, lambda, theta) {
    L <- length(obs)
    n1 <- sum(obs)
    n2 <- L - n1
    gamma1 <- lambda[1] * theta[1]^n1 * (1 - theta[1])^n2
    gamma2 <- lambda[2] * theta[2]^n1 * (1 - theta[2])^n2
