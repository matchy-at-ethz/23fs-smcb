...
\left\lVert\
  \mathbf{y} - \left(
    \alpha\mathbf{X}\hat{\beta}_1 + (1 - \alpha)\mathbf{X}\hat{\beta}_2
  \right)
\right\rVert_2^2 +
\lambda \left\lVert
  \alpha\hat{\beta}_1 + (1 - \alpha)\hat{\beta}_2
...
\left\lVert\
  \mathbf{y} -\left(
    \alpha \mathbf{X} \hat{\beta}_1 + 
    (1 - \alpha) \mathbf{X} \hat{\beta}_2
  \right)
\right\rVert_2^2 +
\lambda \left\lVert
  \alpha\hat{\beta}_1 + 
...
Equation \ref{eq:4} contradicts with Equation \ref{eq:1}, which suggests that
 our assumption $\mathbf{X} \hat{\beta}_1 \neq \mathbf{X} \hat{\beta}_2$ is wrong.
...
```{r combine_res_into_df}
df <- do.call(rbind, lapply(1:length(alphas), function(x) {
  cbind.data.frame(alphas[x], res[[x]]$lambda, res[[x]]$cvm)
}))
...
For the optimal $\alpha$ (here is `r param$alpha`)...
