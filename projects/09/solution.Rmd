...
## (i) Write down all the variables that are d-sepparated from $A$ given ${C, D}$

Just $G$ (because given $A \cancel{\perp} B | D$, hence $B$ is not d-separated from $A$ and consequently $B$'s descendents $F$ and $E$ are also not d-separated).
...
With a p-value of `6.6e-13`, we can reject the null hypothesis that $A|C$ and $B|C$ are not correlated, hence $A$ and $B$ are not conditionally independent given $C$. This also confirms with the uderlying DAG in Figure 2 because $A$ and $B$ explain away each other given $C$.
