...
The result of `cor.test()` on $A$ and $B$ also confirms that there's no
 correlation between $A$ and $B$ (the p-value is `r cor$p.value` > 0.05, so we cannot reject the
...
With a p-value of `r cor2$p.value` < 005, we can reject the null hypothesis that $A|C$ and
