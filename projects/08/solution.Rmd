...
Determine cellular perturbation map $\rho$ where $\rho_{ic} = 1$ if cell $c$ is perturbed by a knowdown of S-gene $i$. 
...
Calculate the responsibilities $\Gamma$ given mixture weights $\pi = (0.44, 0.56)$. Then update the mixture weights. (Well, EM again!)
...
gamma <- rbind(
  diag(pi[1] * exp(L1) / (pi[1] * exp(L1) + pi[2] * exp(L2))),
